...
void PutChar_no_background (uint16_t Xpos, uint16_t Ypos, uint8_t ASCI, uint16_t charColor)
{
		uint16_t i, j;
    uint8_t buffer[16], tmp_char;
    GetASCIICode(buffer, ASCI);  /* 取字模数据 */
        ...
        for(j=0; j<8; j++)
        {
            if( ((tmp_char >> (7 - j)) & 0x01) == 0x01 )
            {
                LCD_SetPoint( Xpos + j, Ypos + i, charColor );  /* 字符颜色 */
						...
						/*	NOTE: background pixels are not drawn 
            else
            {
...
void GUI_Text_no_background (uint16_t Xpos, uint16_t Ypos, uint8_t *str,uint16_t Color)
{
		uint8_t TempChar;
    ...
    {
        TempChar = *str++;  
        PutChar_no_background(Xpos, Ypos, TempChar, Color);    
